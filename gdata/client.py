...
class GDClient(atom.client.AtomPubClient):
	...
	def request(self, method=None, uri=None, auth_token=None,
		...
		# exists since the redirects are only used in the calendar API.
			...
			if redirects_remaining > 0:
				...
				if location is not None:
					...
					return self.request(method=method, uri=location, 
									...
									redirects_remaining=redirects_remaining-1, **kwargs)
				else:
			...
			raise error_from_response('This API operation is not implemented. - Server responded with',	response, NotImplemented)
	...
	def request_client_login_token(
			...
			auth_url=atom.http_core.Uri.parse_uri('https://www.google.com/accounts/ClientLogin'),
		...
		if response.status == 200:
			...
			if token_string is not None:
				...
				raise ClientLoginTokenMissing('Recieved a 200 response to client login request, but no token was present. %s' % (response_body,))
			...
			if captcha_challenge:
				...
				raise error_from_response('Server responded with a 403 code', response, RequestError, response_body)
		elif response.status == 302:
			...
			# authentication URL
			raise error_from_response('Server responded with a redirect', response, BadAuthenticationServiceURL, response_body)
		else:
			raise error_from_response('Server responded to ClientLogin request', response, ClientLoginFailed, response_body)

	RequestClientLoginToken = request_client_login_token

	def client_login(self, email, password, source, service=None, account_type='HOSTED_OR_GOOGLE', auth_url=atom.http_core.Uri.parse_uri('https://www.google.com/accounts/ClientLogin'), captcha_token=None, captcha_response=None):
	...
	def upgrade_token(self, token=None, url=atom.http_core.Uri.parse_uri(
		...
		if token is None: token = self.auth_token
		# We cannot upgrade a None token.
		if token is None: raise UnableToUpgradeToken('No token was provided.')
		if not isinstance(token, gdata.gauth.AuthSubToken):
			raise UnableToUpgradeToken('Cannot upgrade the token because it is not an AuthSubToken object.')
		...
		if response.status == 200:
			...
			raise UnableToUpgradeToken('Server responded to token upgrade request with %s: %s' % (response.status, response.read()))

	UpgradeToken = upgrade_token

	def revoke_token(self, token=None, url=atom.http_core.Uri.parse_uri('https://www.google.com/accounts/AuthSubRevokeToken')):
		...
		response = self.http_client.request(http_request)
		if response.status != 200:
			raise error_from_response('Server sent non-200 to revoke token', response, RequestError, response.read())

		return True

	RevokeToken = revoke_token

	def get_oauth_token(self, scopes, next, consumer_key, consumer_secret=None,	rsa_private_key=None, url=gdata.gauth.REQUEST_TOKEN_URL):
		...
		if rsa_private_key is not None:
			...
			raise MissingOAuthParameters('To request an OAuth token, you must provide your consumer secret or your private RSA key.')

		response = self.http_client.request(http_request)
		response_body = response.read()

		if response.status != 200:
			raise error_from_response('Unable to obtain OAuth request token', response, RequestError, response_body)

		if rsa_private_key is not None:
			return gdata.gauth.rsa_token_from_body(response_body, consumer_key, rsa_private_key, gdata.gauth.REQUEST_TOKEN)
		elif consumer_secret is not None:
			return gdata.gauth.hmac_token_from_body(response_body, consumer_key, consumer_secret, gdata.gauth.REQUEST_TOKEN)
	...
	def delete(self, entry_or_uri, auth_token=None, force=False, **kwargs):
		...
		# Include the ETag in the request if present.
		if force: http_request.headers['If-Match'] = '*'
		...
		if isinstance(entry_or_uri, (str, unicode, atom.http_core.Uri)):
			return self.request(method='DELETE', uri=entry_or_uri,
									http_request=http_request, auth_token=auth_token, **kwargs)
...
def _add_query_param(param_string, value, http_request):
	if value: http_request.uri.query[param_string] = value
...
class GDQuery(atom.http_core.Uri):
	...
	def _get_text_query(self): return self.query['q']

	def _set_text_query(self, value): self.query['q'] = value

	text_query = property(_get_text_query, _set_text_query,	doc='The q parameter for searching for an exact text match on content')
...
class ResumableUploader(object):
	...
	def __init__(self, client, file_handle, content_type, total_file_size, chunk_size=None, desired_class=None):
		...
		if self.chunk_size < self.MIN_CHUNK_SIZE: self.chunk_size = self.MIN_CHUNK_SIZE
		...
		if self.total_file_size <= self.chunk_size:	self.chunk_size = total_file_size

	def _init_session(self, resumable_media_link, entry=None, headers=None,	auth_token=None, method='POST'):
		...
		if entry is None:
			...
			http_request.add_body_part(str(entry), 'application/atom+xml', size=len(str(entry)))
		...
		if headers is not None:	http_request.headers.update(headers)

		response = self.client.request(method=method, uri=resumable_media_link, auth_token=auth_token, http_request=http_request)

		self.upload_uri = (response.getheader('location') or response.getheader('Location'))
	...
	def upload_chunk(self, start_byte, content_bytes):
		...
		http_request.headers['Content-Range'] = ('bytes %s-%s/%s' % (start_byte,start_byte + chunk_size - 1,self.total_file_size))

		try:
			response = self.client.request(method='PUT', uri=self.upload_uri, http_request=http_request, desired_class=self.desired_class)
			return response
	...
	def upload_file(self, resumable_media_link, entry=None, headers=None, auth_token=None, **kwargs):
	...
	def query_upload_status(self, uri=None):
		...
		try:
			response = self.client.request(method='POST', uri=uri, http_request=http_request)
			if response.status == 201:
				return True
			else:
				raise error_from_response('%s returned by server' % response.status, response, RequestError)
		except RequestError, error:
			if error.status == 308:
